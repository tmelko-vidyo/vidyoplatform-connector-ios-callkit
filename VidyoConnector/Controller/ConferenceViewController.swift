...
class ConferenceViewController: UIViewController {
    ...
    struct CallState {
        ...
        var disconnectingWithQuit = true
    }
    ...
    override func viewDidLoad() {
        ...
        connector?.registerLogEventListener(self, filter: "debug@VidyoClient")
        connector?.enableDebug(7777, logFilter: "debug@VidyoClient")
    }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        /* Handle auto-join behavior */
        if connectParams?.autoJoin ?? false {
            connectConference()
        }
    }
...
extension ConferenceViewController: VCConnectorIConnect {
    ...
    func onFailure(_ reason: VCConnectorFailReason) {
        ...
        DispatchQueue.main.async {
            ...
            this.updateCallState()
            this.connectionStatus.text = "Error: \(reason)"
        }
    }
    ...
    func onDisconnected(_ reason: VCConnectorDisconnectReason) {
        ...
        DispatchQueue.main.async {
            ...
            this.connectionStatus.text = "Disconnected: \(reason)"
...
extension ConferenceViewController {
    ...
    private func connectConference() {
        ...
        let status = connector?.connectToRoom(asGuest: connectParams?.portal,
                                 displayName: connectParams?.displayName,
                                 roomKey: connectParams?.roomKey,
                                 roomPin: connectParams?.pin,
                                 connectorIConnect: self)
        
        connectionStatus.text = status ?? false ? "Connecting..." : "Connect error!"
    }
...
// MARK: Required for running logs in console

extension ConferenceViewController: VCConnectorIRegisterLogEventListener {
